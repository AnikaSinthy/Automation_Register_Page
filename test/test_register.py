...
class RegisterTest(Base_Test):
    ...
    def test_valid_register_02(self):
        rp = RegisterPage(self.driver)
        rp.enter_firstname(DATA.VALID_FIRSTNAME_01)
        rp.enter_lastname(DATA.VALID_LASTNAME_01)
        rp.enter_address(DATA.VALID_ADDRESS)
        rp.enter_email(DATA.VALID_EMAIL_01)
        rp.enter_phone(DATA.VALID_PHONE)
        rp.select_gender_male()
        assert rp.assert_male_selected() == True
        rp.select_hobbies_cricket()
        assert rp.assert_cricket_selected() == True
        rp.enter_language()
        rp.select_android(DATA.value)
        time.sleep(2)
        rp.select_country(DATA.country_name)
